# Raspberry Pi Setup (LIVI - MotoCarPlay v2)
...
## 6. LIVI - MotoCarPlay v2 app autostart

The Electron AppImage is deployed to `/home/byron/LIVI/LIVI.AppImage` and
launched by a desktop autostart entry:

```ini
# ~/.config/autostart/livi.desktop
[Desktop Entry]
Type=Application
Name=LIVI - MotoCarPlay v2
Exec=/home/byron/LIVI/LIVI.AppImage
Icon=livi
...
rsync -az dist/LIVI-*-linux-arm64.AppImage \
...
> `pkill -x LIVI` (by process name), **not** `pkill -f LIVI.AppImage`
...
**App runtime config** — `~/.config/LIVI/config.json` is written by the app's
own Settings screen (projection view area, Moto Display backdrop/corners,
connection preferences, audio/mic, key bindings, etc.). It's app-managed, not
part of OS setup — but back it up if you want to preserve tuned values across a
reflash.
